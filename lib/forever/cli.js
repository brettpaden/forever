...
var help = [
  ...
  '  -r  MILLISECONDS   Change how frequently forever checks to see if logs should be rotated',
  ...
  '',
  '[Log Rotation]',
  '  For the -o and -e options you may use a strftime template to enable log rotation. For',
  '  example, to rotate logs daily:',
  '',
  '  forever -o /var/log/myapp-%Y%m%d.log /bin/myapp',
  '',
  '  Will write your app\'s stdout to /var/log/myapp-20121111.log, then rotate when the day changes',
  '  See man 3 strftime for a complete list of supported formats.',
  '  If you require your log rotation to be more accurate than within 60 seconds of your',
  '  format, you may supply and additional argument, -r MILLISECONDS',
  ''
];
...
var argvOptions = cli.argvOptions = {
  ...
  'path':        {alias: 'p'},
  'help':        {alias: 'h'},
  'rotateCheck': {alias: 'r', default: 30000},
...
var getOptions = cli.getOptions = function (file) {
  ...
  // this seems buggy and error prone to me.  Shouldn't this array come from
  // the argvOptions?
  [
...
app.cmd(/usr2 (.+)/, cli.usr2 = function (file) {
  var runner = forever.usr2(file, true);

  runner.on('usr2', function (process) {
    forever.log.info('Forever usr2d process:');
    forever.log.data(process);
  });
...
app.cmd(/kill9 (.+)/, cli.kill9 = function (file) {
  var runner = forever.kill9(file, true);

  runner.on('kill9', function (process) {
    forever.log.info('Forever kill9 process:');
    forever.log.data(process);
  });
...
app.cmd('usr2all', cli.usr2 = function () {
  var runner = forever.usr2All(true);
  runner.on('usr2All', function (processes) {
    if (processes) {
      forever.log.info('Forever stopped processes:');
      processes.split('\n').forEach(function (line) {
        forever.log.data(line);
...
app.cmd('kill9all', cli.kill9all = function () {
  var runner = forever.kill9All(true);
  runner.on('kill9All', function (processes) {
    if (processes) {
      forever.log.info('Forever stopped processes:');
      processes.split('\n').forEach(function (line) {
        forever.log.data(line);
...
app.cmd('gracefulrestartall', cli.gracefulrestartAll = function () {
  var runner = forever.gracefulrestartAll(true);
  runner.on('gracefulrestartAll', function (processes) {
    if (processes) {
      forever.log.info('Forever gracefully restarted processes:');
      processes.split('\n').forEach(function (line) {
        forever.log.data(line);
      });
    }
...
app.cmd(/gracefulrestart (.+)/, cli.gracefulrestart = function (file) {
  var runner = forever.gracefulrestart(file, true);
  runner.on('gracefulrestart', function (processes) {
    if (processes) {
      forever.log.info('Forever gracefully restarted process(es):');
      processes.split('\n').forEach(function (line) {
        forever.log.data(line);
      });
    }
  ...
  runner.on('error', function (err) {
    forever.log.error('Error gracefully restarting process: ' + file.grey);
    forever.log.error(err.message);
  });
});
