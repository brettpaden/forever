...
forever.load = function (options) {
  // This function seems to be the only place configation can happen; as such command line
  // options need to override the default load() below.  A reasonable, forward compatible test
  // would be to see if options are passed to load.  If so, the don't short circuit this function
  // be returning
  var options_passed = options != null ? true : false;
  ...
  if (forever.initialized && 
    forever.config.get('root') === options.root &&
...
forever._debug = function () {
  ...
  // out of order processing test
  if (!debug) {
    forever.config.set('debug', true)
  }
  // this is what you really want to do ... make sure we don't start
  // multiple instances of the same transport
  if (forever.log.transports.file == undefined) {
...
forever.kill9 = function (target, format) {
  return stopOrRestart('kill9', 'kill9', format, target);
};
forever.usr2 = function (target, format) {
  return stopOrRestart('usr2', 'usr2', format, target);
};
...
forever.gracefulrestart = function (target, format) {
  return stopOrRestart('gracefulrestart', 'gracefulrestart', format, target);
};
...
forever.gracefulrestartAll = function (format) {
  return stopOrRestart('gracefulrestart', 'gracefulrestartAll', format);
};
...
forever.kill9All = function (format) {
  return stopOrRestart('kill9', 'kill9All', format);
};
forever.usr2All = function (format) {
  return stopOrRestart('usr2', 'usr2All', format);
};
